<script>
	...
	import {
		...
		getUsersCountByRole
	} from '$lib/api/main-backend-requests/dashboard.js';
				...
				options: {
					plugins: {
						legend: {
							labels: {
								color:
									getComputedStyle(document.documentElement).getPropertyValue('--dashboard-text') ||
									'#222'
							}
						}
					}
...
<div
	class="flex flex-col gap-10 px-6 py-8 md:px-12 bg-gradient-to-br from-white to-slate-100 dark:from-zinc-900 dark:to-slate-950 min-h-screen rounded-3xl"
>
	<div
		class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-slate-300 dark:border-slate-700 pb-4"
	>
		<div>
			<h1 class="text-3xl md:text-4xl font-bold text-blue-600 dark:text-blue-400 drop-shadow">
				Dashboard Fixium
			</h1>
			<p class="text-slate-600 dark:text-slate-300 mt-1">
				👤 <strong>{usuario}</strong> <span class="mx-2 text-gray-400">|</span> 🛡️
				<strong>{rol ? rol.replace('ROLE_', '') : ''}</strong>
			</p>
		</div>
		<div
			class="flex items-center gap-3 text-sm text-slate-600 dark:text-slate-400 text-right leading-relaxed"
		>
	...
	<div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
		<div
			class="chart-card bg-white dark:bg-slate-800 rounded-3xl p-8 shadow-lg flex flex-col items-center justify-center border border-slate-200 dark:border-slate-700 transition hover:shadow-xl hover:border-blue-300 dark:hover:border-blue-500"
		>
			<h3
				class="text-xl font-bold text-slate-700 dark:text-slate-200 mb-6 text-center tracking-tight"
			>
				Tickets por estado
			</h3>
