<script>
	...
	} from '$lib/api/main-backend-requests/workshop.js';
...
{:else}
	<form
		...
		class="max-w-md mx-auto mt-8 bg-white shadow-xl rounded-2xl p-8 flex flex-col gap-6 border border-gray-100 dark:border-gray-700 dark:bg-gray-800"
	>
		<h2 class="text-2xl font-bold mb-2 text-center text-blue-700 dark:text-blue-400">
			Actualizar detalles del taller
		</h2>
		{#if error}
		...
		<div>
			...
			<input
				...
				bind:value={name}
				required
				class="w-full border border-gray-300 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-400 shadow-sm dark:bg-gray-700 dark:border-gray-600 dark:text-gray-300"
			/>
		</div>
		<div>
		...
		<button
			...
			disabled={!formValid || !formChanged}>Actualizar</button
		>
	</form>
{/if}
