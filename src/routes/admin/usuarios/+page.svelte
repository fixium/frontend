<script>
	...
	function startHold() {
		if (emailConfirmation !== userToDelete.username || isDeleting) return;
		holdProgress = 0;
...
<h1 class="text-3xl font-extrabold text-blue-700 mb-8 text-center tracking-tight">
...
{#if showModal}
	<div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
		<div class="bg-white rounded-3xl shadow-lg p-8 w-[400px] dark:bg-gray-800 dark:text-white">
			...
			<div class="mt-6">
				<p class="text-center text-gray-800 mb-4 dark:text-gray-300">
				...
				<input
					...
					class="w-full border-4 border-black text-center text-lg rounded-full px-4 py-3 focus:outline-none dark:bg-gray-700 dark:text-white"
			...
			<div class="mt-6 flex flex-col space-y-4">
				<button
					...
					class="w-full rounded-full py-3 text-white text-lg font-semibold transition relative overflow-hidden select-none
        {emailConfirmation !== userToDelete.username || isDeleting
						? 'bg-gray-400 cursor-not-allowed'
						: ''}"
					disabled={emailConfirmation !== userToDelete.username || isDeleting}
					style={emailConfirmation === userToDelete.username && !isDeleting
						? `background: linear-gradient(to right, #ef4444 ${holdProgress * 100}%, #22c55e ${holdProgress * 100}%);`
						: ''}
				>
					<span class="relative z-10">
						...
						{:else}
							{holdProgress > 0 && holdProgress < 1 ? 'Eliminando...' : 'Confirmar'}
						{/if}
					</span>
					{#if emailConfirmation === userToDelete.username && !isDeleting}
						<div
							class="absolute inset-0 rounded-full pointer-events-none transition-all duration-100"
							style="background: #ef4444; width: {holdProgress * 100}%; opacity: 0.3; z-index: 1;"
						></div>
					{/if}
				</button>
				<button
