<script>
	...
	async function enviarLog() {
		...
		try {
			...
			respuesta =
				resTemp.choices?.[0]?.message?.content ||
				'El log es demasiado largo y no se pudo procesar completamente. Intenta con un log más corto o revisa el contenido manualmente.';
		} catch (e) {
...
<main class="flex items-start justify-center py-8">
	<div class="relative shadow-xl rounded-xl p-8 w-full max-w-2xl border border-blue-100">
		<h1
			class="text-3xl font-extrabold text-blue-700 dark:text-blue-300 mb-4 flex items-center gap-2"
		>
			<svg
		...
		<div class="mb-4 flex gap-4">
			<label class="flex items-center gap-2 cursor-pointer select-none text-dark dark:text-white">
				<input type="radio" bind:group={useFile} value={false} class="peer sr-only" />
				<span
          ...
          peer-checked:border-blue-600 peer-checked:bg-blue-600 transition-colors duration-150"
			...
			<label class="flex items-center gap-2 cursor-pointer select-none text-dark dark:text-white">
				<input type="radio" bind:group={useFile} value={true} class="peer sr-only" />
				<span
					class="w-5 h-5 rounded-full border-2 border-blue-400 flex items-center justify-center
          peer-checked:border-blue-600 peer-checked:bg-blue-600 transition-colors duration-150"
				>
		...
		{#if !useFile}
			<label for="logContent" class="font-semibold mb-2 block text-gray-700 dark:text-gray-300"
				>Pega aquí el contenido del archivo <code class="bg-blue-100 px-1 rounded text-blue-700"
					>panic-full.ips</code
				>:</label
			>
			<textarea
			...
			></textarea>
		{:else}
			<label for="fileInput" class="font-semibold mb-2 block text-gray-700 dark:text-gray-300"
				>Selecciona el archivo <code class="bg-blue-100 px-1 rounded text-blue-700"
					>panic-full.ips</code
				>:</label
			>
				...
				>
					...
					>
						<path
							stroke-linecap="round"
							stroke-linejoin="round"
							d="M7 16v-4a4 4 0 018 0v4M12 12v8m0 0l-3-3m3 3l3-3"
						/>
					</svg>
		...
		{#if error}
			<div
				class="text-red-700 bg-red-100 border border-red-200 rounded p-3 mt-4 flex items-center gap-2"
			>
				<svg
		...
		{#if respuesta}
			<div class="mt-6 bg-green-50 border border-green-200 rounded-xl p-5 shadow-inner">
				...
				<pre
					class="bg-white p-3 rounded-lg whitespace-pre-wrap text-gray-800 font-mono text-sm overflow-x-auto">{respuesta}</pre>
			</div>
		{/if}
		{#if isLoading}
			<div
				class="absolute inset-0 bg-white/80 flex flex-col items-center justify-center z-20 rounded-xl"
			>
