<script lang="ts">
	...
	const messages = writable<
		{ role: 'technician' | 'assistant' | 'user'; content: string; copied?: boolean }[]
	>([]);
	let isLoading = false;
	...
	async function sendMessage() {
		...
		try {
			...
			const assistantMessage =
				data.choices?.[0]?.message?.content || 'No se recibió una respuesta válida.';
...
<main class="fixed inset-0 flex flex-col from-blue-100 via-white to-gray-200 overflow-hidden">
	<div class="flex flex-col items-center">
		<h1 class="text-4xl font-extrabold mb-6 mt-8 text-blue-700 dark:text-blue-300 drop-shadow">
			Asistente Técnico
		</h1>
	</div>

	<div
		class="chat-container flex flex-col gap-4 flex-grow overflow-y-auto px-2 md:px-0 py-4 max-w-2xl mx-auto w-full"
	>
		...
		{#if isLoading}
			<div
				class="self-start bg-white border border-gray-200 rounded-3xl px-5 py-3 max-w-xl flex items-center gap-2 shadow-md ml-2"
			>
	...
	<div class="flex items-end justify-center w-full max-w-2xl mx-auto px-2 md:px-0 pb-6">
		<div
			class="flex items-center rounded-3xl bg-white shadow-lg border border-gray-200 p-3 w-full gap-2"
		>
			<textarea
